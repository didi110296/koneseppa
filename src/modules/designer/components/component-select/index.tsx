...
const ComponentSelect = ({ component, title, infoTitle, infoText, setSelected: setParentSelected, selections }: { component: string; title: string; infoTitle: string; infoText: string; setSelected: any; selections: {
  ...
  const { data } = useGetDesignerOptions(component);
  const [selected, setSelected] = useState(0);
  const [multiSelectedIndex, setMultiSelectedIndex] = useState(0);
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    setParentSelected(Array.isArray((data || [])[selected]) ? ((data || [])[selected] as any)[multiSelectedIndex] : (data || [])[selected]);
                      ...
                      { Array.isArray(product) ? 
                        (
                          <ComponentOptionMultiple
                            ...
                            selected={Array.isArray((data || [])[selected]) ? ((data || [])[selected] as any)[multiSelectedIndex] : (data || [])[selected]}
                            onClick={() => setSelected(index)}
                            setMultiIndex={setMultiSelectedIndex}
